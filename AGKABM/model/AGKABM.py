...
class AGKABM(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        self.attention4 = ELA(in_channels=embed_dims[1])
        self.attention3 = ELA(in_channels=embed_dims[0])
        self.attention2 = ELA(in_channels=embed_dims[0] // 2)
        self.attention1 = ELA(in_channels=embed_dims[0] // 4)

        self.decoder1 = D_ConvLayer(embed_dims[2] + embed_dims[1], embed_dims[1])
        self.decoder2 = D_ConvLayer(embed_dims[1] + embed_dims[0], embed_dims[0])
        self.decoder3 = D_ConvLayer(embed_dims[0] + embed_dims[0] // 2, embed_dims[0] // 2)
        self.decoder4 = D_ConvLayer(embed_dims[0] // 2 + embed_dims[0] // 4, embed_dims[0] // 4)
    ...
    def forward(self, x):
        ...
        for _, blk in enumerate(self.dblock1):
            out_flat = blk(out_flat, H, W)
        out = out_flat.transpose(1, 2).reshape(B, -1, H, W)

        out = F.relu(F.interpolate(out, scale_factor=(2, 2), mode='bilinear', align_corners=False))

        att = self.attention3(t3)
